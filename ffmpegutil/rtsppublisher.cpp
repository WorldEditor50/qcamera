...
void RtspPublisher::enableNetwork()
{
    /* init network */
    if (isInited == false) {
...
void RtspPublisher::start(int width, int height, const std::string &url)
{
    ...
    /* output context */
    int ret = avformat_alloc_output_context2(&outputContext, nullptr, "RTSP", url.c_str());
    if (ret != 0) {
...
void RtspPublisher::encode(unsigned char *data)
{
    std::lock_guard<std::mutex> guard(mutex);
    if (state != STATE_STREAMING) {
        return;
